...
PODNAME=$(kubectl -n nttdata-dach-techblog-secrets-demo get pods | grep secrets-demo | cut -d' ' -f 1 ) 
echo "Starting port-forward to POD ${PODNAME} on port 8080 ..."
kubectl -n nttdata-dach-techblog-secrets-demo port-forward pod/${PODNAME} 8080:8080 &
PID="$!"
echo "To stop port forwarding run kill $PID"